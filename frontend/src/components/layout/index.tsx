...
export function Sidebar() {
  ...
  return (
    <aside className="w-sidebar h-screen bg-[#0E0F11] border-r border-nexus-outline-variant flex flex-col fixed left-0 top-0 z-50">
      {/* Brand */}
      <div className="h-16 flex items-center px-6 gap-3 border-b border-nexus-outline-variant cursor-pointer" onClick={() => handleNav('/')}>
        <div className="w-6 h-6 bg-nexus-primary rounded-none" />
        <span className="font-syne font-bold text-lg tracking-tight">NEXUS</span>
      </div>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              className={clsx(
                'group flex items-center gap-3 px-6 py-3 text-sm font-medium transition-all relative w-full text-left',
            ...
            >
              ...
              <item.icon size={18} className={clsx('transition-colors', isActive ? 'text-nexus-primary' : 'group-hover:text-white')} />
      ...
      {/* User */}
      <div className="p-6 border-t border-nexus-outline-variant flex items-center gap-3">
        <div className="w-8 h-8 rounded-full bg-nexus-surface-variant flex items-center justify-center border border-nexus-outline-variant">
...
export function Header() {
  ...
  return (
    <header className="h-16 border-b border-nexus-outline-variant bg-nexus-bg/80 backdrop-blur-md flex items-center justify-between px-8 sticky top-0 z-40 ml-sidebar">
      {/* Search */}
