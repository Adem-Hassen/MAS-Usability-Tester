...
\begin{itemize}
    \item \textbf{Supervisor Node (incl.\ Verification):} Routed to \code{gpt-5.4} via OpenAI API. The temperature was set to 0.3 for structured reasoning, with a maximum token limit of 2048. The supervisor also assumes the verification role, re-evaluating patched pages after each correction loop.
    \item \textbf{Persona Simulation Nodes:} Routed to \code{deepseek-v4-pro} via DeepSeek API. The temperature was set to 0.7 to support diverse exploratory path generation, with a maximum token limit of 512.
    \item \textbf{Recommender and Conflict Resolver Nodes:} Routed to \code{opus-4.6} via Anthropic API. The recommender temperature was set to 0.7 for creative patch synthesis; the resolver temperature was set to 0.2 for deterministic mediation.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}l c r c r c@{}}
    ...
    \midrule
    Supervisor (incl.\ Verification) & GPT-5.4          & 4,828  & 17\%  & 1,230 & 30\% \\
    Persona Simulator               & Deepseek-v4-pro  & 20,448 & 72\%  & 1,640 & 40\% \\
    Recommender                     & Opus 4.6         & 2,272  & 8\%   & 820   & 20\% \\
    Conflict Resolver               & Opus 4.6         & 852    & 3\%   & 410   & 10\% \\
    \midrule
...
To manage the operating costs and latency of this high-volume phase, \versimux{} employs a hybrid model routing strategy. The persona simulation nodes are routed to Deepseek-v4-pro, a cost-efficient, high-throughput model with strong sequential tool-use performance. The reasoning-heavy orchestration nodes---the Supervisor (which also performs post-patch verification)---are routed to GPT-5.4 for its superior structured reasoning and instruction-following accuracy, while the Recommender and Conflict Resolver are routed to Opus~4.6 for best-in-class code generation and multi-turn argumentation. Because Deepseek-v4-pro is significantly cheaper and has lower latency than the frontier models, routing 72\% of the input tokens to this model results in substantial cost savings without compromising the depth of the simulation. Conversely, GPT-5.4 and Opus~4.6 are reserved for the complex analysis, patch synthesis, conflict mediation, and verification tasks, which collectively represent only 28\% of the input token budget. This routing design ensures that the system scales efficiently for large-scale web applications while maintaining high precision and low regression rates.
